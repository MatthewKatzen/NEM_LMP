...
mpa_year_fuel_region <- mpa %>%  filter(settlementdate < ymd_hms("2019-01-01 00:00:00 UTC"), 
    ...
    filter(fuel_type != "Battery",
           !(station %in% hydro_stations$station)) %>% 
...
mpa_year_fuel_region %>% 
    ggplot(aes(x = year, y = total_overcomp/1000000000, colour = fuel_type)) +
    geom_line(size = 1.5) +
    facet_wrap(~ state) +
    xlab("Year") +
    ...
    ggsave("Output/Charts/TO.png", width = 10, height = 5)
...
mpa_year_fuel_region %>% 
    ggplot(aes(x = year, y = adj_total_overcomp/1000000, colour = fuel_type)) +
    geom_line(size = 1.5) +
    facet_wrap(~ state) +
    xlab("Year") +
    ...
    theme(text = element_text(size = 12))+
    labs(colour = "Fuel Type") +
    ggsave("Output/Charts/ATO.png", width = 10, height = 5)
...
mpa_year_fuel_age %>% 
    ...
    ggplot(aes(x = year, y = ave_overcomp, group = duid, colour = age)) + 
    geom_line(size = 2) + 
    facet_wrap(~ state) +
    xlab("Year") +
