...
#get data
constraint_data <- c(201801:201912) %>% map(~rhs.fun(.x) %>% clean_names()) %>% rbindlist()
fwrite(constraint_data, "D:/NEM_LMP/Data/Cleaned/Misc/constraint_data.csv")
...
#### GRAPHS
#### 
constraint_data %>% mutate(time = paste0("2020-01-01 ", format(settlementdate, "%H:%M:%S")) %>% ymd_hms(),
...
yearmonths <- constraint_data$genconid_effectivedate %>% 
